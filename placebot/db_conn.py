...
class DbConn:
    ...
    def update_user(self, author_id: int, username: str) -> None:
        ...
        self.cursor.execute("""INSERT INTO users(authorID, username) VALUES (:author_id, :username)
                            ON CONFLICT (authorID) DO UPDATE SET username=:username""", params)
        self.conn.commit()
    ...
    def check_existing_tweets(self, author_id: int) -> bool:
        ...
        """
        self.cursor.execute("SELECT EXISTS(SELECT 1 FROM tweets WHERE authorID = (?));", (author_id,))
    ...
    def get_most_recent_tweet(self, author_id: int) -> int:
        ...
        """
        self.cursor.execute("SELECT tweetID FROM tweets WHERE authorID = (?) ORDER BY tweetID DESC LIMIT 1;", (author_id,))
        since = self.cursor.fetchone()[0]
        return since
    
    def write_tweets(self, tweets: list[Tweet], author_id: int) -> None:
